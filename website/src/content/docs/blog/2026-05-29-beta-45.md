...
category: release
excerpt: v2.0.0-beta.45 replaces `Alchemy.Secret` and `Alchemy.Variable` with plain `effect/Config`, and ships two new Worker abstractions — `Cloudflare.RpcWorker` and `Cloudflare.RpcDurableObjectNamespace` — that take a typed Effect RPC schema directly and hand back typed clients.
---
...
+    // usable here in Init AND captured as a binding
+    const client = createClient(Redacted.value(apiKey));

     return {
       fetch: Effect.gen(function* () {
-        const apiKey = yield* API_KEY; // had to re-yield at runtime
-        // ...
+        // use `client` / `apiKey` directly
       }),
     };
   }),
 );
...
`RpcServer.toHttpEffect(schema)` Effect — no `{ fetch }` wrapper,
no transport plumbing.
...
no public URL and no schema re-declaration:
...
The [RpcDurableObjectNamespace tutorial](/tutorial/cloudflare/rpc-durable-object)
...
- **Cross-script Durable Object binding** — the primitive under
  `RpcDurableObjectNamespace`'s `Class.from(Worker)`
...
- **`Cloudflare.Queue` accepts `Duration.Input`** for time props
...
- [RpcWorker tutorial](/tutorial/cloudflare/rpc-worker)
- [RpcDurableObjectNamespace tutorial](/tutorial/cloudflare/rpc-durable-object)
