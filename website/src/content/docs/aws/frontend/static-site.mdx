...
The build is memoized on a hash of its inputs: by default all files under
`path` (filtered by your gitignore rules) plus the nearest package-manager
lockfile. If nothing changed since the last deploy, the command is skipped
entirely. Tune the hash with `build.include`, `build.exclude`, and
