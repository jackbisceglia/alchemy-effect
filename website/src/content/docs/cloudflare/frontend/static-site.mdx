...
assets. The build is a plain shell command (a `Command.Build` resource under
the hood), so it works for anything that produces a directory of files — a
...
explicitly when stdout detection fails.
